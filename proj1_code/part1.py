...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    kernel = [c * (1 / (np.exp(((i - mean) ** 2) / (2 * sigma ** 2)))) for i in range(0, ksize)]
    Sum = sum(kernel)
    for i in range(len(kernel)):
        kernel[i] = kernel[i] / Sum
    res = np.zeros((ksize, 1))
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    a = create_Gaussian_kernel_1D(ksize, sigma)
    b = create_Gaussian_kernel_1D(ksize, sigma).T
    kernel = np.dot(a, b)
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    m, n, c = image.shape
    k, j = filter.shape
    pad = np.zeros((m + 2 * (k // 2), n + 2 * (j // 2), c))
    pad[k // 2:k // 2 + m, j // 2:j // 2 + n] = image
    filtered_image = np.zeros((m, n, c))
    for i in range(c):
        for a in range(m):
            for b in range(n):
                filtered_image[a, b, i] = np.sum(pad[a:a + k, b:b + j, i] * filter)
...
def create_hybrid_image(
    ...
    # fo = open("cutoff_frequency.txt", "r+")
    # List = fo.read(5)
    # fo.close()
    cutoff_frequency = 7  # List[0]
